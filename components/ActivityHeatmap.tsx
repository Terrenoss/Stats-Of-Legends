  ...
  return (
    <div className="bg-[#121212] border border-white/5 rounded-[2rem] p-6 shadow-xl">
       <div className="flex justify-between items-end mb-4">
         <h3 className="text-gray-400 text-xs uppercase font-bold tracking-widest">{t.recentActivity} <span className="text-[#555] ml-2">{t.last120Days}</span></h3>
       </div>
       
       <div className="flex gap-2 text-[10px] text-gray-500 font-mono mb-2 ml-8">
          <span>{t.aug}</span>
          <span className="ml-8">{t.sep}</span>
          <span className="ml-8">{t.oct}</span>
          <span className="ml-8">{t.nov}</span>
       </div>
       ...
       <div className="flex items-center justify-between mt-4 pt-4 border-t border-white/5 text-xs text-gray-400 font-bold">
          <div className="flex gap-4">
            <span><span className="text-white">{t.gamesPlayedCount}</span> {t.gamesPlayedText}</span>
            <span><span className="text-white">262</span> {t.hoursPlayedText}</span>
          </div>
