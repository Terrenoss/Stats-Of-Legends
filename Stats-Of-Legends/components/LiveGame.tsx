...
export const LiveGame: React.FC<LiveGameProps> = ({ summonerName }) => {
  ...
  if (loading) return <div className="h-96 flex items-center justify-center"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-lol-gold"></div></div>;
  ...
  const getChampImg = (id: number) => MOCK_CHAMPIONS.find(c => c.id === id)?.imageUrl || `https://ddragon.leagueoflegends.com/cdn/14.1.1/img/champion/Aatrox.png`;
        ...
        {gameData.participants.filter((p: any) => p.teamId === teamId).map((p: any, i: number) => (
            <div key={i} className={`flex items-center gap-3 p-3 rounded-xl border ${p.summonerName === summonerName ? 'bg-lol-gold/10 border-lol-gold' : 'bg-[#18181b] border-white/5'}`}>
                <div className="relative">
                    <img src={getChampImg(p.championId)} className="w-10 h-10 rounded-lg border border-gray-700" alt="Champion" />
                    <div className="absolute -bottom-1 -right-1 flex gap-0.5">
                       <div className="w-4 h-4 bg-black rounded border border-gray-600"></div> {/* Spell 1 Placeholder */}
                       <div className="w-4 h-4 bg-black rounded border border-gray-600"></div> {/* Spell 2 Placeholder */}
                    </div>
                </div>
                <div>
                    <div className={`font-bold text-sm ${p.summonerName === summonerName ? 'text-lol-gold' : 'text-white'}`}>{p.summonerName}</div>
  ...
  return (
    <div className="animate-fadeIn">
       ...
       <div className="bg-[#121212] border border-white/5 rounded-[2rem] p-6 mb-8 flex items-center justify-between shadow-xl relative overflow-hidden">
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-64 h-full bg-lol-gold/5 blur-3xl"></div>
              ...
              <div className="flex gap-1">
                  {gameData.bannedChampions.map((ban: any, i: number) => (
                      <div key={i} className="w-8 h-8 rounded border border-white/10 bg-black/50 grayscale opacity-70">
                          <img src={getChampImg(ban.championId)} className="w-full h-full object-cover opacity-60" alt="Ban" />
                      </div>
                  ))}
              </div>
          </div>
