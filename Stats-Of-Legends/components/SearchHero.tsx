    ...
    <div className="relative overflow-hidden bg-[#050505] py-24 sm:py-32 border-b border-white/5">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 text-center">
        <div className="inline-block mb-4 px-4 py-1.5 rounded-full bg-lol-gold/10 border border-lol-gold/20 text-lol-gold text-xs font-bold tracking-widest uppercase shadow-glow-gold">
          {info ? `${info.season} ${info.split}` : 'Season 2025 Split 1'}
        </div>
          ...
          {isFocused && (suggestions.length > 0 || (isStorageReady && recentSearches.length > 0)) && (
            <div className="absolute left-0 right-0 top-[calc(100%+8px)] bg-[#121212] border border-white/10 rounded-2xl shadow-2xl text-left z-20 overflow-hidden animate-fadeIn">
              ...
              {suggestions.length > 0 && (
                 <>
                   <div className="px-4 py-2 text-[10px] uppercase text-gray-500 font-bold bg-[#18181b]">Suggestions</div>
                   {suggestions.map((s, idx) => (
                    <button
                      key={`s-${idx}`}
                      type="button"
                      className="w-full px-6 py-3 text-sm text-gray-200 hover:bg-white/5 flex items-center justify-between border-b border-white/5 last:border-0 transition-colors"
                      onClick={() => handleSuggestionClick(s)}
                    >
                      <span className="font-bold">{s.gameName}<span className="text-gray-500 font-normal">#{s.tagLine}</span></span>
                      <span className="text-xs text-lol-gold uppercase tracking-wider">Summoner</span>
                    </button>
                  ))}
                 </>
              )}
              ...
              {suggestions.length === 0 && recentSearches.length > 0 && (
                <>
                  <div className="px-4 py-2 text-[10px] uppercase text-gray-500 font-bold bg-[#18181b] flex items-center gap-2">
                  ...
                  {recentSearches.map((r, idx) => (
                     <button
                       key={`r-${idx}`}
                       type="button"
                       className="w-full px-6 py-3 text-sm text-gray-200 hover:bg-white/5 flex items-center justify-between border-b border-white/5 last:border-0 transition-colors group/item"
                       onClick={() => handleRecentClick(r)}
                     >
                       <div className="flex items-center gap-2">
                          <span className="font-bold">{r.name}<span className="text-gray-500 font-normal">#{r.tag}</span></span>
                          <span className="text-[10px] bg-white/10 px-1.5 rounded text-gray-400">{r.region}</span>
                       </div>
                       <div 
                         onClick={(e) => removeRecent(e, idx)}
                         className="p-1 rounded-full hover:bg-white/20 text-gray-500 hover:text-white transition-colors"
                       >
                          <X className="w-3 h-3" />
                       </div>
                     </button>
          ...
          <div className="mt-8 flex flex-wrap justify-center gap-3 relative z-10">
            {REGIONS.map(region => (
              <button 
                ...
                className={`
                  px-4 py-1.5 text-xs font-bold rounded-full border transition-all duration-300 uppercase tracking-wider cursor-pointer
