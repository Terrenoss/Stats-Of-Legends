...
import { Search, Plus, Swords, Wand2, Shield, Zap, Crosshair, Gauge, Hand } from 'lucide-react';
import { Item, Stats } from '../../types';
import { BUILDER_ITEMS } from '../../constants';
...
export const ItemCatalog: React.FC<ItemCatalogProps> = ({ onItemSelect, t }) => {
  ...
  const handleDragStart = (e: React.DragEvent, item: Item) => {
    e.dataTransfer.setData('sourceType', 'catalog');
    ...
    <div className="lg:col-span-3 flex flex-col gap-4 h-[800px] lg:h-auto">
        ...
        >
          ...
          {filteredItems.length === 0 && (
              <div className="text-center text-gray-500 py-10 text-sm absolute top-4 w-full">No items match filters.</div>
          )}
        </div>
    </div>
  );
