...
import { TRANSLATIONS, CURRENT_PATCH_FALLBACK, DEFAULT_SEASON_INFO } from './constants';
import { SummonerProfile, Match, Language, Region, GameMode, HeatmapDay, DetailedChampionStats, Teammate, SeasonInfo } from './types';
...
function App() {
  ...
  const [matches, setMatches] = useState<Match[]>([]); // toujours un tableau
  const [isSearching, setIsSearching] = useState(false);
  const [currentLang, setCurrentLang] = useState<Language>('FR');

  // Nouveaux états pour remonter les vraies données liées au profil
  const [heatmapData, setHeatmapData] = useState<HeatmapDay[]>([]);
  const [championStats, setChampionStats] = useState<DetailedChampionStats[]>([]);
  ...
  const [currentPatch, setCurrentPatch] = useState<string>(CURRENT_PATCH_FALLBACK);
  const [seasonInfo, setSeasonInfo] = useState<SeasonInfo>(DEFAULT_SEASON_INFO);
  ...
  React.useEffect(() => {
    async function loadPatch() {
      try {
        const res = await fetch('/api/dd/champions?patch=latest&locale=fr_FR');
        if (!res.ok) return;
        const json = await res.json();
        if (json.patch) {
          setCurrentPatch(json.patch);
          // heuristique simple: Season = année du patch, Split = 1 ou 2 selon le mois courant
          const year = new Date().getFullYear();
          const month = new Date().getMonth() + 1;
          const split = month <= 6 ? 'Split 1' : 'Split 2';
          setSeasonInfo({ season: `Season ${year}`, split });
        }
      } catch {
        // fallback: garder valeurs par défaut
      }
    }
    loadPatch();
  }, []);

  const handleSearch = async (query: string, region: Region) => {
    setIsSearching(true);
    setCurrentView('home');
    ...
    try {
      const res = await fetch(`/api/riot/summary?summonerName=${encodeURIComponent(summonerName)}&tag=${encodeURIComponent(tagName || region)}&region=${encodeURIComponent(region)}`);
      if (!res.ok) {
      ...
      setCurrentProfile(data.profile as SummonerProfile);
      setMatches(data.matches as Match[]);
      setHeatmapData(data.heatmap ?? []);
      setChampionStats(data.champions ?? []);
      setTeammates(data.teammates ?? []);
    } catch (err: any) {
      console.error('Failed to fetch riot summary', err);
      setCurrentProfile(null);
      setMatches([]);
      setHeatmapData([]);
      setChampionStats([]);
      setTeammates([]);
      setErrorMessage(err.message || 'Erreur lors de la récupération des données Riot.');
    } finally {
  ...
  const getFilteredMatches = () => {
    ...
    if (matchFilter === 'ALL') return base;
    if (matchFilter === 'SOLO') return base.filter(m => m.gameMode === GameMode.SOLO_DUO);
    if (matchFilter === 'FLEX') return base.filter(m => m.gameMode === GameMode.FLEX);
    return base;
  };
  ...
  const renderContent = () => {
    ...
    if (!currentProfile) {
      return (
        <div className="animate-fadeIn">
          <SearchHero onSearch={handleSearch} lang={currentLang} seasonInfo={seasonInfo} />
          {errorMessage && (
            <div className="max-w-2xl mx-auto mt-4 px-4">
              <div className="bg-red-900/40 border border-red-500/60 text-red-200 text-sm rounded-2xl px-4 py-3">
                {errorMessage}
              </div>
            </div>
          )}
    ...
    return (
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fadeIn">
        <button onClick={() => handleNavigate('home')} className="mb-6 text-sm text-gray-500 hover:text-white transition flex items-center gap-1">← {t.back}</button>
        ...
        {isSearching ? (
          ...
          <>
            ...
            {profileTab === 'overview' && (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 space-y-6">
                   ...
                   <ActivityHeatmap data={heatmapData} lang={currentLang} />
            ...
            {profileTab === 'champions' && (
               <ChampionsTable champions={championStats} lang={currentLang} />
            )}
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-200 bg-lol-dark selection:bg-lol-red selection:text-white">
      <Navbar currentView={currentView} onNavigate={handleNavigate} currentLang={currentLang} onSetLang={setCurrentLang} currentPatch={currentPatch} />
      <main className="flex-grow">{renderContent()}</main>
      <footer className="bg-[#080808] border-t border-white/5 py-8 mt-auto">
        <div className="max-w-7xl mx-auto px-4 text-center text-gray-600 text-sm">
          <p className="font-display tracking-widest uppercase text-xs font-bold text-gray-700">Stats Of Legends</p>
          <p>© 2024 - Inspiré par l'univers de Riot Games.</p>
        </div>
      </footer>
    </div>
  );
