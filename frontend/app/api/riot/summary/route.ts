...
console.log('[riot] Using RIOT_API_KEY prefix:', RIOT_KEY ? String(RIOT_KEY).slice(0, 10) + '...' : 'undefined');
...
export async function GET(req: NextRequest) {
  try {
    ...
    // 1) Compte Riot ID (gameName, tagLine, puuid)
    ...
    if (!accountRes.ok) {
      // Ne pas casser toute la page : renvoyer une erreur lisible au front
    ...
    // Profil de base à partir de Riot ID, on le complétera si possible
    const profile: any = {
      name: account.gameName || summonerName,
    ...
    try {
      // 2) Infos invocateur LoL (level, icon)
      ...
      // On renverra quand même le profil de base avec account
    }
