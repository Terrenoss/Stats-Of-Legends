...
function approximateBaseFromTooltip(spell: any): number[] {
  ...
  // Essayer de récupérer les tokens principaux de dégâts (ordre de priorité grossier)
  const keys = ['qdamage', 'wdamage', 'edamage', 'rdamage', 'totaldamage', 'damage'];

  // DDragon expose souvent ces valeurs dans spell.datavalues ou spell.vars supplémentaires selon les champions
  const dv = spell.datavalues || spell.dataValues || {};

  for (const key of keys) {
    const val = dv[key];
    if (!val) continue;
    // Peut être un tableau [10,20,30,40,50] ou une string "10/20/30/40/50"
...
export async function GET(req: NextRequest) {
  try {
    ...
    // Tenter d'utiliser un backend externe si DATA_BASE est défini
    if (DATA_BASE) {
      try {
        ...
        // on tombera sur le CDN dessous
      }
    }

    // 1) On charge le listing global des champions
    ...
    // 2) Pour chaque champion, on va chercher la fiche détaillée pour récupérer les spells
    for (const c of entries as any[]) {
      ...
      try {
        ...
        if (dr.ok) {
          ...
          if (champData && Array.isArray(champData.spells)) {
            spells = champData.spells.map((s: any, idx: number) => {
              ...
              // Si malgré tout aucun chiffre exploitable, on met une courbe générique en fonction du maxRank
              ...
              if (Array.isArray(s.vars)) {
                for (const v of s.vars) {
                  ...
                  if (v.link === 'attackdamage' || v.key === 'b' || v.link === 'attackdamage') {
                    const coeff = Array.isArray(v.coeff) ? v.coeff[0] : v.coeff;
        ...
        // si on ne peut pas charger les spells, on garde un tableau vide
      }
