...
if (!RIOT_KEY) {
  // On garde cette vérification au chargement pour éviter des erreurs silencieuses.
  // En prod tu peux remplacer par un log/monitoring plutôt qu'un throw au import si besoin.
...
async function riotFetchJson<T>(url: string, revalidateSeconds = 60): Promise<T> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`RIOT_${res.status}: ${body}`);
  }
