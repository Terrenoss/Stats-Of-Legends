...
export class SpectatorService {
    static async getRankFromCache(puuid: string) {
        try {
    ...
    static async fetchSummonerFallback(participant: any, routing: string) {
    ...
    static async fetchRankFromRiot(participant: any, routing: string) {
        if (!participant.puuid) return null;
    ...
    static async enrichParticipant(participant: any, routing: string) {
        let rank = 'UNRANKED';
        let displayName = participant.riotId ? participant.riotId.split('#')[0] : participant.summonerName;

        // 1. Check Cache
        if (participant.puuid) {
            const cached = await this.getRankFromCache(participant.puuid);
            if (cached) {
                return {
                    ...participant,
        ...
        const fallback = await this.fetchSummonerFallback(participant, routing);
        if (fallback) {
            displayName = fallback.displayName;
        }

        // 3. Fetch Rank from Riot
        const riotRank = await this.fetchRankFromRiot(participant, routing);
    ...
    static async getInferredRoles(gameData: any) {
    ...
    static resolveSummoner(searchParams: URLSearchParams): { gameName: string | null, tagLine: string | null, region: string } {
    ...
    static async getActiveGame(gameName: string, tagLine: string, region: string) {
        const { routing, regionalRouting } = this.getRegionRouting(region);
        ...
        if (!accountRes.ok) {
            if (accountRes.status === 404) return { error: 'Summoner not found', status: 404 };
            return { error: 'Riot API Error (Account)', status: accountRes.status };
        }
        ...
        if (spectatorRes.status === 404) {
            return { noActiveGame: true };
        }

        if (!spectatorRes.ok) {
            const body = await spectatorRes.text();
            return { error: `Riot API Error (Spectator): ${spectatorRes.status} - ${body}`, status: spectatorRes.status };
        }

        const gameData = await spectatorRes.json();

        // 3. Enrich Participants
        const participantsWithRanks = await mapWithConcurrency(gameData.participants, 1, (p) => this.enrichParticipant(p, routing));

        // 4. Infer Roles
        const { championsById, championRoles } = await this.getInferredRoles(gameData);
