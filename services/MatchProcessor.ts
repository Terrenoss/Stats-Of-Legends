        ...
        try {
            ...
            for (const p of info.participants) {
                ...
                if (role && role !== 'Invalid') {
                    ...
                    // Timeline logic (Only if we can fetch it or if it's provided?)
                    // NOTE: jsonData from Match table usually doesn't include timeline.
                    // If we want deep stats (skill order, build path), we need timeline.
                    // For now, if jsonData is provided (from Profile), we might miss Timeline data unless we fetch it.
                    // Optimization: If jsonData is provided but no timeline, we might skip deep stats or fetch timeline separately.
                    // Let's try to fetch timeline if not present, but be robust if it fails.

                    try {
                        // We always need timeline for deep stats. 
                        // If we are in "Profile" mode, we might not want to block on this or use extra quota.
                        // But the user asked for "useful for all systems". Deep stats are useful.
                        // Let's fetch timeline.
                        let routing = 'europe';
                        if (region.startsWith('na') || region.startsWith('br') || region.startsWith('la')) routing = 'americas';
                        if (region.startsWith('kr') || region.startsWith('jp')) routing = 'asia';
